...
#include "../../raspberrypi/raspi_driver.hpp"
            ...
            class RegisterBlock {
                public:
                    ...
                    RegisterBlock(unsigned int mmio_start_addr) {
                        GPFSEL1 = reinterpret_cast<volatile unsigned int*>(mmio_start_addr + 0x04);
                        GPPUD = reinterpret_cast<volatile unsigned int*>(mmio_start_addr + 0x94);
                        GPPUDCLK0 = reinterpret_cast<volatile unsigned int*>(mmio_start_addr + 0x98);
                        GPIO_PUP_PDN_CNTRL_REG0 = reinterpret_cast<volatile unsigned int*>(mmio_start_addr + 0xE4);
                    }
            };

...
namespace Driver::BSP::BCM {
    volatile uint32_t* MBOX_STATUS = reinterpret_cast<volatile uint32_t*>(0x3F00B898);
    volatile uint32_t* MBOX_WRITE = reinterpret_cast<volatile uint32_t*>(0x3F00B8A0);
    volatile uint32_t* MBOX_READ = reinterpret_cast<volatile uint32_t*>(0x3F00B880);
    ...
    int call(uint8_t ch) {
            ...
            uintptr_t ptr = reinterpret_cast<uintptr_t>(&mbox);
    ...
    char* UART::compatible() {
        return "brcm,bcm2xxx-pl011-uart";
    }
    ...
    void UART::UartConsole::printChar(char character) {
        uart->putc(character);
    }
    ...
    char UART::UartConsole::readChar() {
        return '\0';
    }
}

...
struct MMIO {
  ...
  MMIO(unsigned long start, unsigned long rng_start, unsigned long gpio_start,
       unsigned long uart_start)
      : START(start), RNG_START(rng_start), GPIO_START(gpio_start),
        PL011_UART_START(uart_start) {}
};
struct Map {
  ...
  static MMIO getMMIO() {
#if BOARD == bsp_rpi3
    return MMIO(0x3F000000, 0x3F000000 + RNG_OFFSET, 0x3F000000 + GPIO_OFFSET,
                0x3F000000 + UART_OFFSET);
#elif BOARD == bsp_rpi4
    return MMIO(0xFE000000, 0xFE000000 + RNG_OFFSET, 0xFE000000 + GPIO_OFFSET,

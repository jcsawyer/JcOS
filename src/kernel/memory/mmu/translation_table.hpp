...
namespace Memory
{
  ...
  struct TableDescriptor
  {
    ...
    static TableDescriptor fromNextLevelTableAddr(uint64_t phys_next_lvl_table_addr)
    {
      ...
      //info("physNextLvlTableAddr: %010x, shifted: %d", phys_next_lvl_table_addr, shifted);
      desc.value = (shifted & 0xFFFFFFFFFFF) << SHIFT_64KiB
          | 0b11; // Set valid and table bits
      //info("phys_next_lvl_table_addr: %d, shifted: %d, desc: %d", phys_next_lvl_table_addr, shifted, desc.value);
      return desc;
    }
  };
...
public:
    ...
    static uint64_t fromAttributeFields(const AttributeFields &attribute_fields) {
        ...
        if (attribute_fields.accessPermissions == AccessPermissions::ReadOnly) {
            desc |= 0b10ULL << 6;
        } else if (attribute_fields.accessPermissions == AccessPermissions::ReadWrite) {
            desc |= 0b00ULL << 6;
        }
    ...
    static PageDescriptor fromOutputAddr(uint64_t phys_output_addr, const AttributeFields& attribute_fields) {
        ...
        desc.value = (shifted & 0xFFFFFFFFFFF) << SHIFT_64KiB
            | (1 << 10) | 0b11 // Set valid and table bits

...
namespace Memory
{
  ...
  template <size_t NUM_SPECIAL_RANGES>
  class KernelVirtualLayout
  {
  public:
    KernelVirtualLayout(size_t max, TranslationDescriptor layout[NUM_SPECIAL_RANGES]) : maxVirtualAddrInclusive(max)
    {
    ...
    int virtAddrProperties(size_t virtAddr, size_t &outputAddr, AttributeFields &attributes) const
    {
    ...
    void printLayout() const
    {
      for (size_t i = 0; i < NUM_SPECIAL_RANGES; i++)
      {
        ...
        const char *attr;
        if (inner[i].attributeFields.memAttributes == MemAttributes::CacheableDRAM)
        {
          attr = "C";
        }
        else if (inner[i].attributeFields.memAttributes == MemAttributes::Device)
        {
          attr = "Dev";
        }
        const char *acc_p;
        if (inner[i].attributeFields.accessPermissions == AccessPermissions::ReadOnly)
        {
          acc_p = "RO";
        }
        else if (inner[i].attributeFields.accessPermissions == AccessPermissions::ReadWrite)
        {
          acc_p = "RW";
        }

        const char *xn = inner[i].attributeFields.executeNever ? "PXN" : "PX";
        info("      0x%08X - 0x%08X | %-3s | %-3s %s %-3s | %-3s", inner[i].virtualRangeStart, inner[i].virtualRangeEnd, size_human_readable_ceil(size), attr, acc_p, xn, inner[i].name);
      }
    }

...
pub fn uart_init() void {
    ...
    r = mmio_read(gpio.GPFSEL1);
    r &= @intCast(~(@as(u32, @intCast((7 << 12) | (7 << 15))))); // Clear GPIO 14, 15
    r &= @intCast(~(@as(u32, @intCast((7 << 12) | (7 << 15))))); // Clear GPIO 14, 15
    r |= (2 << 12) | (2 << 15); // Set alt5 for GPIO 14, 15
    mmio_write(gpio.GPFSEL1, r);

    mmio_write(gpio.GPPUD, 0); // Disable pull-up/down
    delay(150);

    mmio_write(gpio.GPPUDCLK0, (1 << 14) | (1 << 15)); // Enable pins 14, 15
    delay(150);

    mmio_write(gpio.GPPUDCLK0, 0); // Flush GPIO setuppub

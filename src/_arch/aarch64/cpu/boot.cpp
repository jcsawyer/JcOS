...
void prepare_el2_to_el1_transition(uint64_t phys_boot_core_stack_end_exclusive_addr) {
    ...
    asm volatile("msr CNTVOFF_EL2, %[value]"
                 :
                 : [value] "r"(0));
    ...
    asm volatile("msr HCR_EL2, %[value]"
                 :
                 : [value] "r"(1 << 31)); // RW::EL1IsAarch64

    // Simulate exception return
    // Fake a saved program status where all interrupts are masked and SP_EL1 is used as the stack pointer
    uint64_t spsr_value = (1 << 9) | // Mask D (Debug)
                          (1 << 8) | // Mask A (SError)
                          (1 << 7) | // Mask I (IRQ)
                          (1 << 6) | // Mask F (FIQ)
                          0b0101;       // M::EL1h
...
extern "C" void _start_cpp(uint64_t phys_boot_core_stack_end_exclusive_addr) {
    ...
    // Use eret to "return" to EL1 and execute kernel_init
    asm volatile("eret");
}